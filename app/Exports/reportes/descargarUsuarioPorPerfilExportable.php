...
class descargarUsuarioPorPerfilExportable implements FromCollection, WithDrawings,  WithEvents, WithCustomStartCell, WithHeadings {
    /**
    * @return \Illuminate\Support\Collection
    */

    // recibe parametros para realizar la consulta que pobla el reporte
    public function __construct($id_perfil = null) {
    ...
    public function registerEvents(): array { 
        return [AfterSheet::class => function (AfterSheet $event) {
            ...
            $perfil = Perfil::select('nombre')->find($this->id_perfil);

            // titulo del reporte
            $event->sheet->setCellValue('A6', 'Reporte de Usuarios por Perfil '.$perfil->nombre)->getStyle('A6')->applyFromArray($estilos_titulo_reporte);
    ...
    public function collection() {
        return collect(Usuarios::rptUsuarioPerfil($this->id_perfil));
    }
