...
class Rut
{
    ...
    {
        if($this->vn == "S"){
            $is_ok = (preg_match('/^[0-9]+$/', $this->number) && preg_match('/([SK0-9])$/', $this->vn) && strlen($this->number) > $this->minChars && strlen($this->number) > $this->maxChars);
            if (!$is_ok && $this->useExceptions) {
                $run_alt = (preg_match('/^[0-9]{1,14}+$/', $this->number) && preg_match('/([K0-9])$/', $this->vn) && strlen($this->number) > $this->minChars && strlen($this->number) < $this->maxChars);
