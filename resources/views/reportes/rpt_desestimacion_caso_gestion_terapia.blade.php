...
<div class="card p-4 shadow-sm">
	...
	<div class="row">
		...
		<div class="col-2 text-right">
				<button class='btn btn-success' id="xls_reporte" name="xls_reporte" onclick="descargarReporteCasosDesestimadosTerapeuta();">
	...
	<div class="row">
		<div class="table-responsive">
			<table class="table table-striped table-hover encabezado_datatable cell-border w-100" cellspacing="0" id="tabla_desestimacion_caso_terapeuta">
				<thead>
				<tr>
					<th style="text-align: center;width: 20%">Terapeuta</th>
...
<script type="text/javascript">
	$(document).ready( function () {
		...
		tabla_desestimacion_caso_terapeuta = $('#tabla_desestimacion_caso_terapeuta').DataTable({
			"language"	: { "url": "{{ route('index') }}/js/dataTables.spanish.json"},	
			 ...
			 footerCallback: function ( row, data, start, end, display ) {
	            ...
	            var columnas = tabla_desestimacion_caso_terapeuta.columns(':visible').count();
	...
	function descargarReporteCasosDesestimadosTerapeuta(){
		...
		fec_ini = $('#fec_ini').val();
		fec_fin = $('#fec_fin').val();
		window.location.assign("{{ route('rptExportDesestimacionDeCasoTerapeuta') }}/" + comunas+ "/" + fec_ini + "/" + fec_fin);
		//window.location.assign("{{ route('rptExportDesestimacionDeCasoTerapeuta') }}");
	}
