<!DOCTYPE html>
<html lang="en">
<style>
    * {
        font-size: x-small;
    }
    th {
        background-color: #f7f7f7;
        border-color: #959594;
        border-style: solid;
        border-width: 1px;
        text-align: center;
    }

    .bordered td {
        border-color: #959594;
        border-style: solid;
        border-width: 1px;
    }

    table {
        margin-top:1rem;
        border-collapse: collapse;
    }

    /* Para sobrescribir lo que está en div-table.css */
    .divTableCell,
    .divTableHead {
        padding: 0px !important;
        border: 0px !important;
    }
    header{
        position: absolute;
        top: -10px;
        left: 0px;
        right: 0px;
        margin-bottom: 2rem;
    }

</style>
<body>   
    <header>
        <img  alt ="Logo Defensoria de la niñez" src ="data:image/png;base64," style="height: 100px;">
    </header>

    <div class="main">
        <h2 align="center">Instrumento de Línea de Base y Línea de Salida.</h2>
        <br>
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th width="100%" colspan="6">I. IDENTIFICACIÓN</th>
            </tr>
            <tbody>
                <tr>    
                    <td>Nombre y Apellido</td>
                    <td colspan="5">{{$nombre}}</td>
                </tr>
                <tr>
                    <td>Rut</td>
                    <td colspan="5">{{$rut.'-'.$dv}}</td>
                </tr>
                <tr>
                    <td>Dirección</td>
                    <td>
                        <label>Calle</label>
                        <hr>
                        {{$calle}}
                    </td>
                    <td>
                        <label>N°</label>
                        <hr>

                        {{$numero_calle}}
                    </td>
                    <td>
                        <label>Block</label>
                        <hr>

                        {{$bloc}}
                    </td>
                    <td>
                        <label>Depto</label>
                        <hr>

                        {{$depto}}
                    </td>
                    <td>
                        <label>Comuna</label>
                        <hr>

                        {{$comuna}}
                    </td>
                </tr>
                <tr>
                    <td>Teléfono</td>
                    <td colspan="5">{{$telefono}}</td>
                </tr>
                <tr>
                    <td>Correo electronico</td>
                    <td colspan="5">{{$correo}}</td>
                </tr>
                <tr>
                    <td>Edad</td>
                    <td colspan="5">{{$edad}}</td>
                </tr>

            </tbody>
        </table>
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th colspan="3">II. SERVICIOS Y PRESTACIONES(1)</th>
            </tr>
            <tr>
                <th colspan="3">1. ¿CUÁL DE LOS SERVICIOS PÚBLICOS CONOCE A NIVEL COMUNAL?</th>
            </tr>
            <tbody> 
                <tr>
                    <td>Servicios</td>
                    <td>Marque con una "X" aquellos que usted conoce a nivel comunal.</td>
                    <td>Marque con una "X" aquellos que utiliza usted o una persona de su familia.</td>

                </tr>
              @foreach($res1_tab1 as $resp)
                <tr>
                    <td>{{$resp->nombre}}</td>
                    @if($resp->lb_res_pre1 == 1)
                    <td>X</td>
                    @else
                    <td></td>
                    @endif

                    @if($resp->lb_res_pre2 == 1)
                    <td>X</td>
                    @else
                    <td></td>
                    @endif
                </tr>
              @endforeach 
                <tr>
                    <td>1.1. "Otro", ¿cuál?</td>
                    <td colspan="2">{{$otro1}}</td>
                </tr> 
            </tbody>
        </table>
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th colspan="3">2. DE LOS SIGUIENTES PROGRAMAS SOCIALES Y PRESTACIONES ¿CUÁLES CONOCE USTED A
                    NIVEL DE SU COMUNA?</th>
            </tr>
            <tbody>
                <tr>
                    <td>Programas sociales y
                        prestaciones
                        </td>
                    <td>Marque con una "X" aquellos que usted conoce a nivel comunal.</td>
                    <td>Marque con una "X" aquellos que utiliza usted o una persona de su familia.</td>
                </tr>
              @foreach($res2_tab2 as $resp)
                <tr>
                    <td>{{$resp->nombre}}</td>
                    @if($resp->lb_res_pre1 == 1)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif

                    @if($resp->lb_res_pre2 == 1)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif
                </tr>
              @endforeach  
                <tr>
                    <td>2.1. "Otro", ¿cuál?</td>
                    <td colspan="2">{{$otro2}}</td>
                </tr>
            </tbody>
        </table>
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th colspan="3">3. ¿CUÁL DE LOS SIGUIENTES SERVICIOS SE ENCUENTRAN EN SU SECTOR?</th>
            </tr>
            <tbody>
                <tr>
                    <td>Servicios</td>
                    <td>Marque con una "X" aquellos que usted conoce a nivel comunal.</td>
                    <td>Marque con una "X" aquellos que utiliza usted o una persona de su familia.</td>
                </tr>
              @foreach($res3_tab3 as $resp)
                <tr>
                    <td>{{$resp->nombre}}</td>
                    @if($resp->lb_res_pre1 == 1)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif

                    @if($resp->lb_res_pre2 == 1)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif
                </tr>
              @endforeach
                <tr>
                    <td>3.1. "Otro", ¿cuál?</td>
                    <td colspan="2">{{$otro3}}</td>
                </tr>
            </tbody>
        </table>
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th colspan="3">III. ORGANIZACIÓN COMUNITARIA
                </th>
            </tr>
            <tr>
                <th colspan="3">1. ¿CUÁL DE LOS SIGUIENTES BIENES COMUNITARIOS CONOCES EN TU SECTOR?</th>
            </tr>
            <tbody>
                <tr>
                    <td>Bienes</td>
                    <td>Marque con una "X" aquellos que usted conoce a nivel comunal.</td>
                    <td>Marque con una "X" aquellos que utiliza usted o una persona de su familia.</td>
                </tr>
              @foreach($res4_tab4 as $resp)
                <tr>
                    <td>{{$resp->nombre}}</td>
                    @if($resp->lb_res_pre1 == 1)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif

                    @if($resp->lb_res_pre2 == 1)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif
                </tr>
              @endforeach
                <tr>
                    <td>1.1. "Otro", ¿cuál?</td>
                    <td colspan="2">{{$otro4}}</td>
                </tr>
            </tbody>
        </table>
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th colspan="4">2. ¿CUÁL DE LAS SIGUIENTES ORGANIZACIONES CONOCE EN SU SECTOR?</th>
            </tr>
            <tbody>
                <tr>
                    <td>Organizaciones</td>
                    <td>Marque con una "X" aquellos que usted conoce a nivel comunal.</td>
                    <td>Marque con una "X" aquellos que utiliza usted o una persona de su familia.</td>
                    <td>Marque con una "X" aquellas de las cuales usted conoce a sus dirigentes.</td>
                </tr>
                @foreach($res5_tab5 as $resp)
                <tr>
                    <td>{{$resp->nombre}}</td>
                    @if($resp->lb_res_pre1 == 1)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif

                    @if($resp->lb_res_pre2 == 1)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif

                    @if($resp->lb_res_pre3 == 1)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif
                </tr>
              @endforeach
                <tr>
                    <td>2.1. "Otro", ¿cuál?</td>
                    <td colspan="3">{{$otro5}}</td>
                </tr>
                <tr>
                    <td>2.2. Si usted no
                        participa en
                        organizaciones ¿Le
                        gustaría participar en
                        alguna organización?
                        </td>
                    <td colspan="3">{{$parte}}</td>
                </tr>
            </tbody>
        </table>
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th colspan="7">IV. DERECHOS Y PARTICIPACIÓN DE LOS NIÑOS, NIÑAS Y ADOLESCENTES</th>
            </tr>
            <tbody>
                <tr>
                    <td>Afirmación</td>
                    <td>1. Muy en desacuerdo</td>
                    <td>2. En desacuerdo</td>
                    <td>3. Ni en desacuerdo / ni de acuerdo </td>
                    <td>4. De acuerdo</td>
                    <td>5. Muy de acuerdo</td>
                    <!-- inicio ch	 -->
                    <td>6. No corresponde</td> 
                    <!-- fin ch -->
                </tr>
            @foreach($res7_tab7 as $resp)
                <tr>
                @foreach($resp as $respuesta)
                    <td>{{$respuesta->nombre}}</td>
                    @if($respuesta->valor == 1)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif

                    @if($respuesta->valor == 2)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif

                    @if($respuesta->valor == 3)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif

                    @if($respuesta->valor == 4)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif

                    @if($respuesta->valor == 5)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif
                    <!-- inicio ch -->
                    @if($respuesta->valor == 6)
                    <td>X</td>
                    @else
                    <td> </td>
                    @endif
                    <!-- fin ch -->
                @endforeach
                </tr>
            @endforeach
            </tbody>
        </table>
    </div>
</body>
</html>