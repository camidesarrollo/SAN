<!DOCTYPE html>
<html lang="en">
<style>
    * {
        font-size: x-small;
    }
    th {
        background-color: #f7f7f7;
        border-color: #959594;
        border-style: solid;
        border-width: 1px;
        text-align: center;
    }

    .bordered td {
        border-color: #959594;
        border-style: solid;
        border-width: 1px;
    }

    table {
        margin-top:1rem;
        border-collapse: collapse;
    }

    /* Para sobrescribir lo que estÃ¡ en div-table.css */
    .divTableCell,
    .divTableHead {
        padding: 0px !important;
        border: 0px !important;
    }
    header{
        position: absolute;
        top: -10px;
        left: 0px;
        right: 0px;
        margin-bottom: 2rem;
    }

</style>
<body>   
    <header>
        <img  alt ="Logo Defensoria de la niÃ±ez" src ="data:image/png;base64," style="height: 100px;">
    </header>

    <div class="main">
    	<br>
        <h2 align="center">Informe Diagnóstico Participativo Comunitario</h2>
        <br>
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th width="100%" colspan="6">I. IDENTIFICACIÓN</th>
            </tr>
            <tbody>
                <tr>    
                    <td colspan="2">Nombre del Gestor/a Comunitario/a a Cargo</td>
                    <td colspan="4">{{$datos->info_resp}}</td>
                </tr>
                <tr>
                    <td colspan="2">Comuna</td>
                    <td colspan="4">{{$datos->info_com}}</td>
                </tr>
                <tr>
                    <td colspan="2">Comunidad Priorizada</td>
                    <td colspan="4">{{$com_pri->com_pri_nom}}</td>
                </tr>                
                <tr>
                    <td colspan="2">Fecha Primer Contacto</td>
                    <td colspan="4">{{$datos->info_fec_pri}}</td>
                </tr>
                <tr>
                    <td colspan="2">Fecha de Término DPC</td>
                    <td colspan="4">{{$datos->info_fec_ter}}</td>
                </tr>

            </tbody>
        </table>
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th colspan="5">II. INTRODUCCIÓN</th>
            </tr>
            <tr>
                <th colspan="5">PRESENTACIÓN INFORME</th>
            </tr>
            <tbody> 
                <tr>
                    <td colspan="5">{{$datos->info_intro}}</td>                   
                </tr>
            </tbody>
            <tr>
                <th colspan="5">VINCULACIÓN DE LA OLN A LA COMUNIDAD</th>
            </tr>
            <tbody> 
                <tr>
                    <td colspan="5">{{$datos->info_vin_oln}}</td>
                    
                </tr>
            </tbody>
            <tr>
                <th colspan="5">CARACTERÍSTICAS GENERALES </th>
            </tr>
            <tbody>
                <tr>
                    <td><b>Actores clave presentes en la comunidad</b></td>
                    <td><b>Organizaciones funcionales (formales y no formales) presentes en la comunidad</b></td>
                    <td><b>Instituciones, ONG y programas presentes en la comunidad</b></td>
                    <td><b>Participación de los NNA en la comunidad</b></td>
                    <td><b>Bienes comunes presentes en la comunidad</b></td>
                </tr>
                @foreach($introduccion as $intro)
                <tr>
                    <td>{{$intro->iden_nom_rep}}</td>
                    <td>{{$intro->organizacion}}</td>
                    <td>{{$intro->instituciones}}</td>
                    <td>{{$intro->iden_num_nna}}</td>
                    <td>{{$intro->bienes}}</td>
                </tr>
                @endforeach
                
            </tbody>
        </table>
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th colspan="5">III. GRUPO DE ACCIÓN
                </th>
            </tr>
            <tr>
                <th colspan="5">CONFORMACIÓN DEL GRUPO DE ACCIÓN
                </th>
            </tr>
            <tbody> 
                <tr>
                    <td colspan="5">{{$datos->info_gru_acc}}</td>
                    
                </tr>
            </tbody>
            <tr>
                <th colspan="5">INTEGRANTES DEL GRUPO DE ACCIÓN
                </th>
            </tr>
            <tbody>
                <tr>
                    <td>Nombres y Apellidos</td>
                    <td>Rut</td>
                    <td>Teléfono</td>
                    <td>Correo Electrónico</td>
                    <td>Rol en el Grupo de Acción</td>
                </tr>
                @foreach($grupoAccion as $grupo)
                <tr>
                    <td>{{$grupo->int_info_nom}}</td>
                    <td>{{$grupo->int_info_rut}}</td>
                    <td>{{$grupo->int_info_tel}}</td>
                    <td>{{$grupo->int_info_cor}}</td>
                    <td>{{$grupo->int_info_rol}}</td>
                </tr>
                @endforeach
            </tbody>
            <tr>
                <th colspan="5">PLANIFICACIÓN DE ACTIVIDADES DEL DIAGNÓSTICO PARTICIPATIVO COMUNITARIO
                </th>
            </tr>
            <tbody> 
                <tr>
                    <td colspan="5">{{$datos->info_plan_act}}</td>
                    
                </tr>
            </tbody>
        </table>
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th colspan="8">IV. EJECUCIÓN DEL DIAGNÓSTICO PARTICIPATIVO COMUNITARIO</th>
            </tr>
            <tbody>
                <tr>
                    <td>Actividad Planificada</td>
                    <td>Actividades Realizadas</td>
                    <td>Destinatarios</td>
                    <td>Medios de Difusión y Convocatoria</td>
                    <td>Metodología</td>
                    <td>Cantidad de Participantes</td>
                    <td>Facilitadores</td>
                    <td>Obstaculizadores</td>
                </tr>
                @foreach($ejecucion as $eje)
                <tr>
                    <td>{{$eje->eje_act_plan}}</td>
                    <td>{{$eje->eje_act_real}}</td>
                    <td>{{$eje->eje_dest}}</td>
                    <td>{{$eje->eje_med_dif}}</td>
                    <td>{{$eje->eje_met}}</td>
                    <td>{{$eje->eje_can_par}}</td>
                    <td>{{$eje->eje_fac}}</td>
                    <td>{{$eje->eje_obst}}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th colspan="3">V. RESULTADOS</th>
            </tr>
            <tbody> 
                <tr>
                    <td colspan="3">{{$datos->info_res}}</td>
                    
                </tr>
            </tbody>
        </table>
        
        <br>
        <br>
        <table border="1" width="80%" cellpading="0" cellspacing="0" align="center">
            <tr>
                <th colspan="3">VI. CONCLUSIONES Y RECOMENDACIONES</th>
            </tr>
            <tbody> 
                <tr>
                    <td colspan="3">{{$datos->info_con_res}}</td>
                    
                </tr>
            </tbody>
        </table>
    </div>
</body>
</html>