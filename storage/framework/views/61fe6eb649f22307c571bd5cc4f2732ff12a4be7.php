<header>
    <nav class="navbar navbar-expand topbar ">
          ...
          <div id="logo" class="m-3" >
            <a href="<?php echo e(env('APP_URL')); ?>"><img src="/img/logo-main-simple.png"></a>
          </div>
          <input type="hidden" value="<?php echo e(route('coordinador.caso.ficha')); ?>" id="ruta_ficha">
		  <input type="hidden" value="<?php echo e(route('coordinador.caso.resumenNNA')); ?>" id="ruta_resumenNNA">
          ...
          <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item m-1">
				<a class="nav-link text-light" href="<?php echo e(route('documentos.gestor.casos')); ?>" id="link-consultas"> <small>Documentos</small></a>            
            </li>
			<!-- //CZ SPRINT 74 -->
			...
			<a class="nav-link dropdown-toggle text-light" id="bell" role="button"  data-toggle="modal" data-target="#ModalNotificaciones" onclick="datosTablaCaso();"> 
				<i class="far fa-bell"></i> <h6 class="text-light" style=" text-align: center;font-size: 11px;"><span id="circle"><?php echo e(Session::get('cantidad')); ?></span></h6>   </a> 
            ...
            <li class="nav-item dropdown m-1">
                <a class="nav-link dropdown-toggle text-light" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                  <small> <?php echo e(Session::get('nombre_usuario')); ?></small>
                </a>
                <!-- Dropdown - User Information -->
                <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="userDropdown">
                  <div class="dropdown-item" href="#">
                    <i class="fa fa-user fa-sm mr-2 text-gray"></i> <?php echo e(ucfirst(Session::get('tipo_perfil'))); ?>

                  </div>
                  <div class="dropdown-item" href="#">
                    <i class="fa fa-map-marker fa-sm mr-2 text-gray"></i>
                    		<!--  -->
							<?php echo e(Session::get('comuna')); ?><br>
                  </div>
                  <?php if(count(Session::get('comunas')) > 1): ?>
                  <div class="dropdown-item">
                <!--   	<i class="fa fa-cog fa-sm mr-2 text-gray"></i>
                  	<i class="oi oi-loop-circular text-gray"></i> -->
                  	 	<i class="oi oi-loop-square text-gray"></i>
                  	<a href="<?php echo e(route('configuracion.comuna.modificar')); ?>" style="text-decoration: none; color: #001c41;">
                  		 &nbsp;Cambiar Comuna
                  	</a>
                  </div>
                  <?php endif; ?>
                  <!--<div class="dropdown-item" href="#">
                    <i class="fa fa-globe fa-sm mr-1 text-gray"></i> <?php echo e(ucwords(mb_strtolower(Session::get('region')))); ?>

                  </div>-->
                  <div class="dropdown-divider"></div> 
                  <a class="dropdown-item" href="<?php echo e(route('logout')); ?>" onclick="logout();">
...
<!-- <header>
	<div class="container">
		<div class="row align-items-center">
			<div class="col-2">
				<a href="<?php echo e(env('APP_URL')); ?>"><img src="/img/logo-main.png" height="80" class="img-fluid"></a>
			</div>
			...
			<div class="col col-md-10 text-right">
				<div class="dropdown">
					...
					<!-- LOGIN 
					<div class="btn-group">
						<button type="button" class="btn btn-info dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?php echo e(Session::get('nombre_usuario')); ?>

						</button>
						<div class="dropdown-menu dropdown-menu-right">
							<button class="dropdown-item" type="button" onclick="window.location.href='<?php echo e(route('logout')); ?>'"><span class="oi oi-x"></span> Salir</button>
						</div>
					</div>
					<!-- LOGIN
...
//CZ SPRINT 74 -->
function cargarNotificacionesTiempoIntervencion(){
	var cantidadNotificacion = 0;
	$("#Cargando_historial_tiempos_intervencion").css("display", "block");
	$("#Cargando_historial_tiempos_intervencion").removeClass('d-none');
	$("#Cargando_historial_tiempos_intervencion").addClass('d-flex');
	var html ="";
	 $.ajax({
		 url: "<?php echo e(route('data.notificacion_tipo_intervencion')); ?>",
		 type: "GET",
	 }).done(function(resp){
		// console.log("obteniendo notificaciones de tiempo de intervencion");
		// $("#txt_numeroNotiTiempo").html(resp.length);
		// if(resp.length == 0){
		// 	$("#mensaje_noti_tiempos_intervencion").removeClass('d-none');
		// 		$("#mensaje_noti_tiempos_intervencion").addClass('d-flex');
		// 		$("#Cargando_historial_tiempos_intervencion").addClass('d-none');
		// 		$("#Cargando_historial_tiempos_intervencion").removeClass('d-flex');
		// }else{
		// 	$("#historial_CasoAsignado").css("display", "block");
		// 		$("#Cargando_historial_tiempos_intervencion").addClass('d-none');
		// 		$("#Cargando_historial_tiempos_intervencion").removeClass('d-flex');
		// 	for(var i= 0; i< resp.length; i++){
		// 	var urlCaso = "<?php echo e(route('coordinador.caso.ficha')); ?>/"+resp[i].run+"/"+resp[i].caso;
		// 	html+=`<div class='notifications-item' onclick="irCaso('${urlCaso}')"   style='cursor: pointer;'>`; 
		// 	html+=`<div class="text">`;
		// 	html+=`<h4>${resp[i].tituloNotificacion}</h4>`;
		// 	html+=`<p>${resp[i].mensaje}</p>`;
		// 	html+=`</div>`;
		// 	html+=`</div>`;
		//  }
		//  $("#notifications_intervencion").html(html);
		// }
	...
	if(cantidadNotificacion >0){
		var tablaHistoria = $('#table_id').DataTable({
			"searching": false,
			"info":     false,
			"ordering": false,
			"lengthChange": false,
...
//CZ SPRINT 74 -->
 function cargarNotificaciones(option){
	 ...
	 $.ajax({
		 url: "<?php echo e(route('buscar.notificaciones')); ?>",
		 type: "GET",
		 data: data
	 }).done(function(resp){
		 if (resp.estado == 1){
...
//CZ SPRINT 74 -->
 function cantidadNotificaciones(){
	...
	var html ="";
	$.ajax({
		url: "<?php echo e(route('data.cantidadNotificaciones')); ?>",
		type: "GET",
	}).done(function(resp){
		console.log(resp)
